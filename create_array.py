...
def create_cross_cost():
    ...
    que = deque([[state2idx(solved[i])[0], 0, 0] for i in range(12)])
    cnt = 0
    phase = 0
    while que:
        cnt += 1
        if cnt % 10000 == 0:
            print(cnt, len(que))
        state_idx, cost, strt_idx = que.popleft()
        n_cost = cost + 1
        for pin_idx in range(strt_idx, 8):
            n_strt_idx = pin_idx + 1
            for twist in range(1, 12):
                n_idx = cross_trans[state_idx][pin_idx * 11 + twist - 1]
                if cross_cost[n_idx] > n_cost:
                    cross_cost[n_idx] = n_cost
                    que.append([n_idx, n_cost, n_strt_idx])
    with open('cross_cost.csv', mode='w') as f:
        writer = csv.writer(f, lineterminator='\n')
        writer.writerow(cross_cost)

def create_corner_cost():
    corner_trans_rev = [[-1 for _ in range(14 * 11)] for _ in range(12 ** 4)]
    for idx in range(12 ** 4):
        if idx % 1000 == 0:
            print(idx)
        state = idx2state(0, 0, idx * 144)
    ...
    while que:
        ...
        state_idx //= 12
        n_cost = cost + 1
        for pin_idx in range(strt_idx, 14):
            n_strt_idx = pin_idx + 1
            pin_num = pins_num_candidate[phase][pin_idx]
            for twist in range(1, 12):
                ...
                if pins_candidate[pin_num][1]:
                    ...
                    n_lower_idx -= twist
                    n_lower_idx %= 12
                n_idx = corner_trans_rev[state_idx][pin_idx * 11 + twist - 1] * 144 + n_upper_idx * 12 + n_lower_idx
